...
class Decoder(nn.Module):
    ...
    def decode(self, inputs, memory, mask=None):
        # Attention RNN
        self.query = self.attention_rnn(
            torch.cat((memory, self.context_vec), -1), self.query)
        self.context_vec = self.attention(self.query, inputs,
                                          self.processed_inputs, mask)
